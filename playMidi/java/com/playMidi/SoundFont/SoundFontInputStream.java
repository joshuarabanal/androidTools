...
public class SoundFontInputStream {
     ...
     * @param out
     */
    public static void readSoundFont(InputStream in, SoundFontMidiTimbreSet out) throws IOException {
        ...
        Log.i("readSoundFont", "file ATTRIBUTE_length:"+(riffLength+4));
        int index = 8;
        if (RiffName.equals("RIFF")) {
            RiffName = Reader.getRiffName(inst);
            if (RiffName.equals("sfbk")) {
                index += readRIFF_SFBK(inst, out, riffLength-4);
            } else {
     ...
     * @return
     */
    private static int readRIFF_SFBK(BufferedInputStream in, SoundFontMidiTimbreSet out, int end) throws IOException {

        int index = 0;
        while (index < end) {
            Log.i("index:"+index,"end:"+end);
            ...
            if (chunkName.equals("LIST")) {
                ...
                index += length+8;
                Log.i("chunk name","LIST"+chunkName+", :"+index+"<"+end);
                switch (chunkName) {
                    case "INFO":
                        Log.i("read info","ATTRIBUTE_length:"+length);
                        readRIFF_SFBK_LIST_INFO_LIST(in, out, length - 4);
                        break;
                    case "sdta":
                        Log.i("read sdta","ATTRIBUTE_length:"+length);
                        readRIFF_SFBK_sdta(in, out, length - 4);
                        break;
                    case "pdta":
                        Log.i("read pdta","ATTRIBUTE_length:"+length);
                        SampleMetas sm = new SampleMetas();
                        readRIFF_SFBK_pdta(in, sm, length - 4);
    ...
    private static void readRIFF_SFBK_sdta(BufferedInputStream in, SoundFontMidiTimbreSet out, int end) throws IOException {
        int start = 0;
        while (start < end) {
            ...
            switch (name) {
                case "smpl":
                    Log.i("read smpl length", ""+length);
                    read_stdasmpl(in, out, length);
                    break;
                default:
    ...
    private static void read_stdasmpl(BufferedInputStream in, SoundFontMidiTimbreSet out, int end) throws IOException {
    ...
    private static void readRIFF_SFBK_pdta(BufferedInputStream in, SampleMetas sampleMetas, int end) throws IOException {
        ...
        while (index < end) {
            ...
            switch (name) {
                ...
                case "shdr":
                    readRIFF_SFBK_ptda_shdr(in, sampleMetas, length);break;
...
private static String generatorOperatorToString(BufferedInputStream in) throws IOException {
    ...
    private static void readRIFF_SFBK_ptda_shdr(BufferedInputStream in, SampleMetas sampleMetas, int end) throws IOException {
        ...
        while(index<end){
            ...
            obj.hSampleName = Reader.getString(in, 20).trim();index += 20;
            obj.start = Reader.getRiffLength(in);index += 4;
            obj.end = Reader.getRiffLength(in);index += 4;
            obj.startLoop = Reader.getRiffLength(in);index += 4;
            obj.endLoop = Reader.getRiffLength(in);index += 4;
            obj.sampleRate = Reader.getRiffLength(in);index += 4;
            obj.originalPitch = in.read();index++;
            obj.pitchCorrection = in.read();index++;
            obj.sampleLink = in.getWord(); index += 2;
